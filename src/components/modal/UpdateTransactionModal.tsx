...
import { useUpdateTransactionMutation } from '../../redux/apiSlices/transactionSlice';
...
const UpdateTransactionModal: React.FC<IPaidModalProps> = ({ open, setOpen, refetch }) => {
    const [form] = Form.useForm();
    const [updateTransaction] = useUpdateTransactionMutation();

    const onFinish: FormProps["onFinish"] = async (values) => {
        await updateTransaction({ id: open?._id, body: values }).unwrap().then(() => {
            form.resetFields();
            setOpen(null);
            refetch();
        })
    ...
    return (
        <div>
            <Modal
                title={<p className='text-[#080808] text-[20px] mb-3'>Update Transaction</p>}
                open={open}
                width={320}
